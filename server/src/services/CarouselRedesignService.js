...
class CarouselRedesignService {
  ...
  async processFiles(files, instagramHandle, userPrompt = '', onProgress = () => {}) {
    ...
    for (const file of files) {
      ...
      try {
        // ── Step 1: Preprocess to PNG for reliable Vision input ───────────────
        await sharp(originalPath).png().toFile(processedImagePath);
